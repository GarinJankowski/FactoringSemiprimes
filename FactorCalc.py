...
def generate_polynomial(n, factor_base, sieving_interval, t_values, min_q, z_list, parity_matrix):
    ...
    q = generate_quadratic_residue_primes(1, n, min_q)[0]
    a = q**2
    b = Tonelli_Shanks(n, q, 2)[0]
    c = (b ** 2 - n) // a
            ...
            for i in range(2):
                ...
                if y_list_copy[factor_i] != 1 and factor_base[-1] < y_list_copy[factor_i] <= factor_base[-1] ** 2:
                    ...
                    if large_prime in partial_relations:
                        ...
                        new_partial_y = y_list[factor_i]

                        new_x = ((a*partial_x+b)**2) * ((a*new_partial_x+b)**2)
                        new_y = partial_y * new_partial_y * (a**2)
...
def generate_polynomial_v2(n, factor_base, sieving_interval, t_values, min_a, z_list, parity_matrix):
    factor_base = factor_base.copy()
    sqrt_n = math.isqrt(n)
    ...
    for i in range(len(factor_base)):
        prime = factor_base[i]
        starting_indices = factor_indices_p[i]
        positive_side = starting_indices.copy()
        negative_side = [k-prime for k in positive_side]
        for m in range(len(positive_side)):
            factor_i = positive_side[m]
            if factor_i < sieving_interval:
                if y_list_copy[factor_i] < 0:
                    y_list_copy[factor_i] *= -1
                    prime_factor_dict[y_list[factor_i]][-1] = 1
                if y_list_copy[factor_i] % prime == 0:
                    prime_factor_dict[y_list[factor_i]][prime] = 0
                    while y_list_copy[factor_i] % prime == 0:
                        y_list_copy[factor_i] //= prime
                        prime_factor_dict[y_list[factor_i]][prime] += 1
                else:
                    # should NEVER happen
                    print(prime, factor_i, y_list[factor_i], y_list_copy[factor_i], n)
                    print("WRONG WRONG WRONG")
                positive_side[m] += prime

            factor_i = negative_side[m]
            if factor_i > -sieving_interval:
                if y_list_copy[factor_i] < 0:
                    y_list_copy[factor_i] *= -1
                    prime_factor_dict[y_list[factor_i]][-1] = 1
    ...
    for i in range(sieving_interval+1):
        if y_list_copy[i] == 1:
            ...
            if tup not in z_list:
                z_list.append(tup)
                prime_factors = prime_factor_dict[y_list[i]]
                # can't actually tell if we need this next line
                prime_factors[a] = 1
                parity_matrix.append(prime_factors_to_parity(prime_factors, factor_base))
...
def step_by_step(n, min_a=0):
    ...
    print("Digits (base 10):", num_digits)

    scary_calculation = (math.e ** (math.sqrt(math.log(n) * math.log(math.log(n))))) ** (math.sqrt(2) / 4)
    base_size = int(scary_calculation)
    # sieving_interval = int(scary_calculation**3/1000)
    sieving_interval = int((num_digits**4)/50)
    if sieving_interval < 10:
        sieving_interval = 10
    ...
    z_list = []
    parity_matrix = []
    a_min = factor_base[-1] + 1
    # a_min = math.sqrt(2*n)/sieving_interval
    if min_a != 0:
        a_min = min_a
    num_polynomials = 0
    while len(z_list) < base_size + 10:
        num_polynomials += 1
        newa = generate_polynomial(n, factor_base, sieving_interval, t_values, a_min, z_list, parity_matrix)
        a_min = newa + 1
    ...
    left_null_space = Matrix.left_null_space(copy.deepcopy(parity_matrix), 2)
    parity_matrix = Matrix.mod_matrix(left_null_space[0], 2)
    new_identity_matrix = Matrix.mod_matrix(left_null_space[1], 2)
    valid_combos = []
    ...
    for combo in valid_combos:
        ...
        square_a_test = 1
        # print("combo: ", combo)
        # TRY DOING THIS BY PRIME FACTOR EXPONENTS
        # WAIT MAYBE ITS JUST NOT A PERFECT SQUARE
        for i in range(len(combo)):
            if combo[i] == 1:
        ...
        if square_a % n != square_b % n:
            print("HEY:", square_a, square_b)
        # extra
        if (root_a % n) == (root_b % n) or (root_a % n) == ((-root_b) % n):
            trivials += 1
            continue
        # factor1 = int(GCD_Stein(abs(square_a - square_b), n))
        ...
        if factor1 != 1 and factor1 != n:
            ...
            for i in range(len(combo)):
                if combo[i] == 1:
                    comb_list.append((z_list[i][0], z_list[i][1]))
                    if z_list[i][0] % n != z_list[i][1] % n:
                        print("HOLD IT")
            print("\t", comb_list)
            print("ROOTS:")
            print("\t", root_a)
            ...
            print("\t", square_a)
            print("\t", square_b)
            print("SQUARED MOD N (SHOULD BE EQUAL):")
            print("\t", square_a % n)
            print("\t", square_b % n)
            print("FAKE SQUARED MOD N (SHOULD BE EQUAL):")
            print("\t", mod_pow(root_a, 2, n))
            print("\t", mod_pow(root_b, 2, n))
            print("MOD N (SHOULD NOT BE EQUAL):")
            print("\t", root_a % n)
            print("\t", root_b % n)
            print("\t", (-root_b) % n)
            # print((-root_b) % n)
            print("A - B:")
            print("\t", abs(root_a - root_b))
            print("GCD A - B:")
            print("\t", GCD_Stein(abs(root_a - root_b), n))
            print("GCD A + B:")
            print("\t", GCD_Stein(root_a + root_b, n))
    print("didn't pass:", trivials)
    print("Only trivial factors found.")
    return None
